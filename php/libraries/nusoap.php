...
function strftime_replaced(string $format, ?int $timestamp = null): string {
	$timestamp = $timestamp ?? time();
	$date = new DateTime("@" . $timestamp); // Create DateTime object from timestamp

	$format_map = [ // Mapping of strftime format codes to DateTime::format() equivalents
		'%a' => 'D',
		'%A' => 'l',
		'%b' => 'M',
		'%B' => 'F',
		'%c' => 'c', // Locale-dependent date and time representation - closest equivalent
		'%C' => '', // Century (not directly supported by DateTime::format) - needs custom logic if needed
		'%d' => 'd',
		'%D' => 'm/d/y',
		'%e' => 'j', // Day of the month, without leading zeros
		'%F' => 'Y-m-d',
		'%g' => 'y', // Year of the week, two digits (not directly supported - use date('o', $timestamp) if needed)
		'%G' => 'Y', // Year of the week, four digits (not directly supported - use date('o', $timestamp) if needed)
		'%h' => 'M', // Same as %b
		'%H' => 'H',
		'%I' => 'g',
		'%j' => 'z', // Day of the year
		'%k' => 'G', // Hour (24-hour clock) without leading zeros
		'%l' => 'h', // Hour (12-hour clock) without leading zeros
		'%m' => 'm',
		'%M' => 'i',
		'%n' => "\n",
		'%p' => 'A',
		...
		'%t' => "\t",
		'%T' => 'H:i:s',
		'%u' => 'w', // Day of the week (Monday=1..Sunday=7) - DateTime::format 'w' is Sunday=0..Saturday=6. Needs adjustment if needed.
		'%U' => '', // Week number of the year (Sunday as the first day of the week) - not directly supported
		'%V' => '', // Week number of the year (Monday as the first day of the week, ISO 8601) - not directly supported
		'%w' => 'w', // Day of the week (Sunday=0..Saturday=6)
		'%W' => '', // Week number of the year (Monday as the first day of the week) - not directly supported
		'%x' => 'm/d/y', // Locale-dependent date representation
		'%X' => 'H:i:s', // Locale-dependent time representation
		'%y' => 'y',
		'%Y' => 'Y',
		'%z' => 'O', // Timezone offset
	...
	while ($i < strlen($format)) {
		if ($format[$i] === '%') {
			$code = $format[$i + 1];
			if (isset($format_map[$format[$i].$code])) {
				$formatted .= $date->format($format_map[$format[$i].$code]);
			} else if (isset($format_map[$code])) {
				$formatted .= $date->format($format_map[$code]);
			} else {
				$formatted .= '%' . $code; // Handle unknown format codes literally
			}
...
class nusoap_base
{
    ...
    function getmicrotime()
    {
        ...
        return strftime_replaced('%Y-%m-%d %H:%M:%S', $sec) . '.' . sprintf('%06d', $usec);
    }
